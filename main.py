# ------------------------------------------------------------------
# DarkAD: A Simple Network for Image Anomaly Detection and Localization (https://openaccess.thecvf.com/content/CVPR2023/papers/Liu_DarkAD_A_Simple_Network_for_Image_Anomaly_Detection_and_Localization_CVPR_2023_paper.pdf)
# Github source: https://github.com/DonaldRR/DarkAD
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        DarkAD_list = methods["get_DarkAD"](imagesize, device)

        models_dir = os.path.join(run_save_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        for i, DarkAD in enumerate(DarkAD_list):
            # torch.cuda.empty_cache()
            if DarkAD.backbone.seed is not None:
                utils.fix_seeds(DarkAD.backbone.seed, device)
            LOGGER.info(
                "Training models ({}/{})".format(i + 1, len(DarkAD_list))
            )
            # torch.cuda.empty_cache()

            DarkAD.set_model_dir(os.path.join(models_dir, f"{i}"), dataset_name)
            if not test:
                i_auroc, p_auroc, pro_auroc = DarkAD.train(dataloaders["training"], dataloaders["testing"])
            else:
                # BUG: the following line is not using. Set test with True by default.
                # i_auroc, p_auroc, pro_auroc =  DarkAD.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
...
def net(
    ...
    def get_DarkAD(input_shape, device):
        DarkADs = []
        for backbone_name, layers_to_extract_from in zip(
            ...
            DarkAD_inst = DarkAD.DarkAD(device)
            DarkAD_inst.load(
                backbone=backbone,
            ...
            DarkADs.append(DarkAD_inst)
        return DarkADs

    return ("get_DarkAD", get_DarkAD)
